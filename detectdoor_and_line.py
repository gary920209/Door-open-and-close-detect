...
def detect_door_frame(input_video_path, output_video_path):
    '''
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        if i % 2 != 0: # Only process every other frame to save time
            continue
        ...
        for result in results:
            ...
            for box, conf, cls in zip(boxes, confs, classes):
                ...
                if cls == 0:
                    ...
                    # save Coordinates for close door and frame
                    close_x1.append(x1)
                    close_y1.append(y1)
                    close_x2.append(x2)
                    close_y2.append(y2)
        ...
        open_x1_med = np.median(open_x1)
        open_y1_med = np.median(open_y1)
        open_x2_med = np.median(open_x2)
        open_y2_med = np.median(open_y2)
        cap.release()
        out.release()
        # resize the frame to 960x960 for hough line detection
    ...
    return open_x1_med, open_y1_med, open_x2_med, open_y2_med

def hough_line_detect(X1,X2,Y1,Y2,video_path, output_path):
    '''
    The function is used to detect straight edges in the video.
    Arg(s):
        X1, X2, Y1, Y2: Coordinates of the door frame
        video_path : Path to the input video
        output_path: Path to the output video (with straight edges marked)
    '''
    # Open the video file
...
x1, y1, x2, y2 = detect_door_frame('03.mp4', '03_door_frame.mp4')
